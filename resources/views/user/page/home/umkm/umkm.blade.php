    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
        @forelse ($umkms as $index => $umkm)
            <a href="{{ route('user.umkm.detail', ['id' => $umkm->id]) }}"
                class="umkm-card group block opacity-0 translate-y-8" style="animation-delay: {{ $index * 100 }}ms">
                ...
                <div
                    ...
                    <div class="p-6 relative">
                        ...
                        <div class="mb-6">
                            @if ($umkm->deskripsi)
                                <p
                                    class="{{ $index % 3 == 0 ? 'text-gray-200' : 'text-gray-700' }} text-base leading-relaxed line-clamp-3">
                                    "{{ Str::limit($umkm->deskripsi, 150) }}"
                                </p>
                            @else
                                <p
                                    class="{{ $index % 3 == 0 ? 'text-gray-200' : 'text-gray-700' }} text-base leading-relaxed">
                        ...
                        <!-- Business Name & Price -->
                        <div class="mb-4">
                            <h3 class="{{ $index % 3 == 0 ? 'text-white' : 'text-gray-900' }} text-xl  mb-2">
                                {{ $umkm->nama_usaha }}
                            </h3>
                            @if (!empty($umkm->harga))
                                <p
                                    class="{{ $index % 3 == 0 ? 'text-green-400' : 'text-green-600' }} text-lg font-semibold">
                        ...
                        <!-- Owner Info -->
                        <div
                            class="flex items-center justify-between pt-4 border-t {{ $index % 3 == 0 ? 'border-gray-700' : 'border-gray-200' }}">
                            <div class="flex items-center gap-3">
                                @if ($umkm->foto_pengusaha)
                                    <img src="{{ asset($umkm->foto_pengusaha) }}" alt="{{ $umkm->nama_pengusaha }}"
                                        class="w-12 h-12 rounded-full object-cover border-2 {{ $index % 3 == 0 ? 'border-gray-600' : 'border-gray-300' }} shadow-md">
                                @else
                                    <div
                                        class="w-12 h-12 rounded-full {{ $index % 3 == 0 ? 'bg-gray-700' : 'bg-gray-200' }} flex items-center justify-center">
                                        <svg class="w-6 h-6 {{ $index % 3 == 0 ? 'text-gray-400' : 'text-gray-500' }}"
                                ...
                                <div>
                                    <p
                                        class="{{ $index % 3 == 0 ? 'text-white' : 'text-gray-900' }} font-semibold text-sm">
