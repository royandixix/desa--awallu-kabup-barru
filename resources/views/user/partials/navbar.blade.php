  ...
  <div class="w-full mx-auto px-4 sm:px-6 lg:px-8">
    <div class="flex justify-between items-center h-16 lg:h-20">
      ...
      <a href="{{ route('user.home') }}" class="flex items-center space-x-2 flex-shrink-0 pl-2">
        <img src="/img/user/logo/barru.webp" alt="Logo Desa Lawallu" class="h-10 w-10 sm:h-12 sm:w-12 object-contain transition-transform hover:scale-105" />
      ...
      <!-- TOGGLE MOBILE -->
      <button id="mobile-toggle" class="md:hidden text-white p-2 hover:bg-white/10 rounded-lg transition-colors" aria-label="Toggle menu">
      ...
      <div class="hidden md:flex items-center space-x-2 font-medium text-white text-sm lg:text-base">
        ...
        <a href="{{ route('user.pengaduan') }}" class="px-3 py-2 rounded-lg hover:text-lime-300 hover:bg-white/10 transition flex items-center gap-1">
          Pengaduan
        </a>
        <a href="{{ route('user.kontak') }}" class="px-3 py-2 rounded-lg hover:text-lime-300 hover:bg-white/10 transition">Kontak</a>

        <a href="/login" class="px-4 py-2 rounded-full bg-gradient-to-r from-lime-400 to-emerald-400 text-teal-900 font-semibold hover:scale-105 hover:shadow-lg transition ml-2">Login</a>
      </div>
    </div>
  </div>

  <!-- MENU MOBILE -->
  <div id="mobile-menu" class="md:hidden bg-teal-950/70 backdrop-blur-xl border-t border-white/10 shadow-inner max-h-0 overflow-hidden transition-all duration-500 ease-in-out">
    <div class="px-4 py-4 space-y-1 text-white text-base font-medium">
      ...
      <a href="{{ route('user.pengaduan') }}" class="block px-4 py-2.5 rounded-lg hover:bg-lime-600/50 transition flex items-center gap-2">
        Pengaduan
      </a>
